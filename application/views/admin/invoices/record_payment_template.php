...
echo render_input('amount', 'record_payment_amount_received', $amount, 'number', ['max' => $amount]); ?>
                    ...
                    <?php
                    $invoiceAllowedPaymentModes = [];
if (!is_null($invoice->allowed_payment_modes)) {
    $invoiceAllowedPaymentModes = unserialize($invoice->allowed_payment_modes);
    if (!is_array($invoiceAllowedPaymentModes)) {
        $invoiceAllowedPaymentModes = [];
    }
}
?>
                    <div class="form-group">
                        ...
                        <select class="selectpicker" name="paymentmode" data-width="100%"
                            ...
                            <?php
        $isAllowed = is_payment_mode_allowed_for_invoice($mode['id'], $invoice->id);
        if ($isAllowed) {
            $totalAllowed++;
        }
        $paymentModeId = (string) $mode['id'];
        $isOverride    = !$isAllowed && !in_array($paymentModeId, array_map('strval', $invoiceAllowedPaymentModes), true);
        ?>
                            <option
                                value="<?= e($mode['id']); ?>"
                                data-payment-mode-allowed="<?= $isAllowed ? '1' : '0'; ?>"
                                <?php if ($isOverride) { ?>
                                data-payment-mode-override="1"
                                data-subtext="Override"
                                <?php } ?>>
                                <?= e($mode['name']); ?>
                            </option>
                            <?php } ?>
                        </select>
                        <p class="text-muted mtop5 hide" id="payment_mode_override_notice">
                            Autogenerated note will be added because this payment mode is different from the invoice payment mode.
                        </p>
                    </div>
                    <?php
                    if ($totalAllowed === 0) {
...
<script>
    $(function() {
        ...
        var $allowedPaymentModes = $sMode.find('option[data-payment-mode-allowed="1"]');
        if ($allowedPaymentModes.length == 1) {
            $sMode.selectpicker('val', $allowedPaymentModes.eq(0).attr('value'));
            $sMode.trigger('change');
        }
        $sMode.on('changed.bs.select change', function() {
            var selected = $(this).find('option:selected');
            $('#payment_mode_override_notice').toggleClass('hide', selected.data('payment-mode-override') != 1);
        });
    });
</script>
