...
class Imap
{
    ...
    public function __construct($mailbox, $username, $password, $encryption = false)
    {
        ...
        try {
            // logActivity('Imap __construct[mailbox: '. $this->mailbox .' , '. $username .']', null);
            // ob_start();
            // var_dump(phpinfo());
            // $str = ob_get_clean();
            // logActivity('Imap __construct[imap_open: '. $str .']', null);
          
           $this->imap    = imap_open($this->mailbox, $username, $password);
           if ($this->imap === false) {
                logActivity('Error connecting to '.$mailbox);
            };
        //    logActivity('Imap __construct[err: '. $e .' ]');
       } catch (Exception $e){
             logActivity('Imap __construct[err: '. $e .' ]');
       }
    }
