...
class Payments_model extends App_Model
{
    ...
    public function add($data, $subscription = false)
    {
        ...
        if (is_staff_logged_in()) {
            ...
            if (!isset($data['note']) && $this->session->has_userdata('payment_admin_note')) {
                $data['note'] = $this->session->userdata('payment_admin_note');
                $this->session->unset_userdata('payment_admin_note');
            }
            $data = $this->append_payment_mode_override_note($data);
            if (isset($data['note'])) {
    ...
    private function append_payment_mode_override_note($data)
    {
        if (empty($data['invoiceid']) || empty($data['paymentmode'])) {
            return $data;
        }

        $invoice = $this->invoices_model->get($data['invoiceid']);
        if (!$invoice || is_null($invoice->allowed_payment_modes)) {
            return $data;
        }

        $allowedPaymentModes = unserialize($invoice->allowed_payment_modes);
        if (!is_array($allowedPaymentModes) || count($allowedPaymentModes) === 0) {
            return $data;
        }

        $paymentModeId = (string) $data['paymentmode'];
        $allowedPaymentModeIds = array_map('strval', $allowedPaymentModes);
        if (in_array($paymentModeId, $allowedPaymentModeIds, true)) {
            return $data;
        }
        ...
        $selectedPaymentModeName = $this->get_payment_mode_name_for_override_note($paymentModeId);
        $invoicePaymentModeNames = [];
        foreach ($allowedPaymentModeIds as $allowedPaymentModeId) {
            $invoicePaymentModeNames[] = $this->get_payment_mode_name_for_override_note($allowedPaymentModeId);
        }

        $overrideNote = 'Payment mode override autogenerated: Da ghi nhan thanh toan bang "'
            . $selectedPaymentModeName
            . '" thay vi payment mode tren hoa don "'
            . implode(', ', array_filter($invoicePaymentModeNames))
            . '" vi khach chuyen vao tai khoan khac.';

        $data['note'] = trim($data['note'] ?? '');
        $data['note'] = $data['note'] === '' ? $overrideNote : $data['note'] . "\n\n" . $overrideNote;

        return $data;
    }

    private function get_payment_mode_name_for_override_note($paymentModeId)
    {
        $paymentMode = $this->payment_modes_model->get($paymentModeId, [], true, true);

        if ($paymentMode && isset($paymentMode->name)) {
