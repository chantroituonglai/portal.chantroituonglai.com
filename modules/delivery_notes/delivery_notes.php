...
if (!function_exists('delivery_note_render_view')) {
    function delivery_note_render_view($relativePath, array $data = [])
    {
        $viewPath = module_views_path(DELIVERY_NOTE_MODULE_NAME, ltrim($relativePath, '/'));

        if (!file_exists($viewPath)) {
            log_message('error', 'Delivery notes view not found: ' . $viewPath);
            return;
        }
...
hooks()->add_action('after_finance_settings_tabs_content', function () {
    delivery_note_render_view('admin/' . DELIVERY_NOTE_MODULE_NAME . '/settings.php');
});
...
hooks()->add_action('after_admin_estimate_preview_template_tab_content_last_item', function ($estimate) {
    delivery_note_render_view('admin/scripts/convert_from_estimate.php', ['estimate' => $estimate]);
});
...
hooks()->add_action('after_admin_purchase_order_preview_template_tab_content_last_item', function ($purchase_order) {
    delivery_note_render_view('admin/scripts/convert_from_purchase_order.php', ['purchase_order' => $purchase_order]);
});
...
hooks()->add_action('after_admin_invoice_preview_template_tab_content_last_item', function ($invoice) {
    delivery_note_render_view('admin/scripts/convert_from_invoice.php', ['invoice' => $invoice]);
});
...
hooks()->add_action('after_email_templates', function () use ($CI) {
    ...
    delivery_note_render_view('admin/email_templates.php', ['templates' => $templates, 'email_type' => $type]);
});
...
hooks()->add_action('after_dashboard_top_container', function () {
    delivery_note_render_view('admin/scripts/dashboard_widget.php');
});
