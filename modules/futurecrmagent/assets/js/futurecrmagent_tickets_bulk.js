(function(){
  ...
  function insertBulkButton(){
    ...
    if(!modalBody || document.getElementById('futurecrmagent-bulk-checkbox')) return;

    var wrapper = document.createElement('div');
    wrapper.className = 'checkbox checkbox-primary';
    wrapper.innerHTML = '<input type="checkbox" id="futurecrmagent-bulk-checkbox">'
      + '<label for="futurecrmagent-bulk-checkbox">Classify with FutureCRM Agent</label>';
  ...
  function showModal(){
    $('#futurecrmagent_classify_modal').one('shown.bs.modal', function(){
      // elevate backdrop behind our modal
      $('.modal-backdrop').addClass('futurecrmagent-backdrop');
    }).modal('show');
    updateProgress(0, 1);
    document.getElementById('futurecrmagent-results-body').innerHTML = '';
  }

  function updateProgress(done, total){
    var pct = total>0 ? Math.round(done*100/total) : 0;
    var bar = document.getElementById('futurecrmagent-progress-bar');
    if(bar){ bar.style.width = pct+'%'; bar.textContent = pct+'%'; }
    var text = document.getElementById('futurecrmagent-progress-text');
    if(text){ text.textContent = done + ' / ' + total; }
  }

  function appendResult(r){
    var tbody = document.getElementById('futurecrmagent-results-body');
    if(!tbody) return;
  ...
  function finishFlow(){
    ...
    try { $('#futurecrmagent_classify_modal').modal('hide'); } catch(e){}
  ...
  function classify(ids){
    ...
    function nextBatch(start){
      ...
      $.post(admin_url + 'futurecrmagent/bulk_classify', { ids: slice })
        .done(function(resp){
  ...
  function hijackConfirmButton(){
    ...
    window.tickets_bulk_action = function(btn){
      var chk = document.getElementById('futurecrmagent-bulk-checkbox');
      if(chk && chk.checked){
