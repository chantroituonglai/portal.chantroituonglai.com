...
class Migration_Version_102 extends App_module_migration
{
    public function up()
    {
        ...
        $actionFields = $CI->db->list_fields($actionsTable);
        if (!in_array('prompt_override', $actionFields)) {
            $CI->db->query('ALTER TABLE `' . $actionsTable . '` ADD COLUMN `prompt_override` TEXT NULL AFTER `permissions`');
            log_message('info', 'Project Agent: Added prompt_override column to actions table');
        }
        ...
        if (!in_array('is_chain_selected', $entryFields)) {
            ...
            log_message('info', 'Project Agent: Added is_chain_selected column to memory entries table');
        }
        if (!in_array('chain_priority', $entryFields)) {
            ...
            log_message('info', 'Project Agent: Added chain_priority column to memory entries table');
        }
        if (!in_array('related_question_id', $entryFields)) {
            ...
            log_message('info', 'Project Agent: Added related_question_id column to memory entries table');
        }
        ...
        if (!$CI->db->table_exists($chainsTable)) {
            ...
            log_message('info', 'Project Agent: Created memory_chains table');
        }

        update_option('project_agent_db_version', 102);
    }
}
