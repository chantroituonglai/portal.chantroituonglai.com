...
// Disabled to consolidate with Geminiai module flow
...
function predix_ticket_created_automated_reply($arg1 = '', $arg2 = '')
{
    if (get_option('predix_ai_autoreply_on_opening_ticket') == '1'):
        ...
        if (!is_null($ticket_info->admin) || true == true) {

         
            $apiKey = get_option('predix_openai_secret_key');
            $message = $ticket_info->message;

            // Bước 1: Kiểm tra xem nội dung ticket có phải là câu hỏi hay không
            $checkPrompt = "Is the following text a question/order/Request consulting service or a notification (example: 'I want to consult website design services' is Question)?  Please respond with 'question' or 'notification': \"" . strip_tags($message) . "\"";

            $openAi = new OpenAi($apiKey);

            $checkResult = $openAi->completion([
            ...
            // Kiểm tra kết quả để quyết định tiếp tục hay không
            log_message('error', 'predix_ticket_created_automated_reply: $checkResult -> '. $_checkResult);
            if (stripos($checkResult->choices[0]->message->content, 'question') === false) {
                $CI->db->update(db_prefix() . 'tickets', array('adminread' => 1, 'status' => 7), array('ticketid' => $arg1));
                return false; // Nếu không phải là câu hỏi, dừng xử lý
            }

          
            $agentTitle = get_option('predix_ai_reply_agent_title');
            $agentName = get_option('predix_ai_reply_agent_name');
            $companyName = get_option('companyname');
            $companyPhone = get_option('invoice_company_phonenumber'); // Số điện thoại của công ty
            $companyAddress = get_option('invoice_company_address'); // Địa chỉ của công ty

            $prompt = "I work as a {$agentTitle} and my name is {$agentName}. The client's company name is {$companyName}. 
            Our company contact information is: Phone - {$companyPhone}, Address - {$companyAddress}. 
            Write a ticket reply about '" . strip_tags($message) . "', that the client wrote. 
            Please do not refer to the client's name or any details about the client and also please reply in the same language that the client wrote the ticket. 
            At the end of the reply, please mention that this is an automated response and that we will contact and support them as soon as possible.";
