...
class Assets_aeiou
{
    public static function getPurchaseData($code)
    {
        ...
        return ($response->success) ? json_decode($response->body) : true;
    }
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            $verified = !empty($token); // Luôn trả về đã xác thực
            $data     = Assets_JWT::decode($token, new Assets_Key($id_data[3], 'HS512'));
    
            // Bỏ qua các điều kiện xác thực
            $seconds           = $data->check_interval ?? 0;
            $last_verification = (int)get_option($module_name.'_last_verification');
    
            try {
                $headers  = ['Accept' => 'application/json', 'Authorization' => $token];
                $request  = Assets_Requests::post(VAL_PROD_POINT, $headers, json_encode([
                    'verification_id'   => $verification_id,
                    'item_id'           => basename($module['headers']['uri']),
                    'activated_domain'  => base_url()
                ]));
                $result   = json_decode($request->body);
                $verified = true; // Bỏ qua kết quả trả về, luôn xác thực
            } catch (Exception $e) {
                $verified = true; // Nếu có lỗi thì vẫn xác thực
            }
    
            // Cập nhật lại thời gian xác thực lần cuối
            update_option($module_name.'_last_verification', time());
    
            // Kiểm tra lần xác thực trước và heartbeat
            $last_verification = (int)get_option($module_name.'_last_verification');
            $heart             = json_decode(base64_decode(get_option($module_name.'_heartbeat')));
            $verified          = true; // Luôn xác thực, bỏ qua điều kiện
    
            // Không deactivate module
            // get_instance()->app_modules->deactivate($module_name);
    
            return $verified; // Luôn trả về đã xác thực
        }
    }
